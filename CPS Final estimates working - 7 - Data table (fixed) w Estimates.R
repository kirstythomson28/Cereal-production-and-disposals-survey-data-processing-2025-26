...
# enter all zeros 
filtered_disposals <- disposals.w.est %>%
...
Data_table_fixed_est_with_totals <- bind_rows(Data_table_fixed_est, Monthly_totals, region_totals, grand_totals)
# Replace NA values with 0 in column 'x'
Data_table_fixed_est_with_totals$count_parish_holdings[is.na(Data_table_fixed_est_with_totals$count_parish_holdings)] <- 0

# 5. arrange the data
Data_table_fixed_est_with_totals <- Data_table_fixed_est_with_totals %>%
  arrange(Month, Region, Crop)

#View(Data_table_fixed_est_with_totals)
